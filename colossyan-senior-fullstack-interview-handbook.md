# Senior Fullstack Engineer Interview Handbook — Colossyan
...
**Как используется / работает** — разработчик даёт стабильные типы компонентов и keys в списках; React сопоставляет узлы по типу и key.
**Минусы и ограничения** — смена key намеренно сбрасывает state; плохие или индексные keys приводят к неверному состоянию и лишним обновлениям.
**Мини-пример** — для списка задач key должен быть id задачи, а не её текущий индекс.
...
**Минусы и ограничения** — Virtual DOM не гарантирует скорость и не отменяет стоимость JavaScript, layout и больших деревьев.
...
**Как используется / работает** — React может начать, прервать и повторить render; useTransition помечает несрочные обновления.
**Минусы и ограничения** — render должен быть чистым и идемпотентным; нельзя рассчитывать, что он выполнится один раз.
...
**Для чего используется** — показывает контролируемый fallback, пока ленивый компонент или совместимый источник данных не готов.
**Как используется / работает** — оборачиваю независимую часть экрана в Suspense с локальным skeleton или loader.
**Минусы и ограничения** — граница, поставленная слишком высоко, скрывает весь экран; не каждый data-fetching подход интегрирован с Suspense.
...
**Как используется / работает** — ставлю boundary вокруг маршрута или рискованного виджета, логирую ошибку и даю retry.
**Минусы и ограничения** — не ловит ошибки обработчиков событий, async-кода и SSR; их нужно обрабатывать отдельно.
...
**Для чего используется** — пропускает повторный рендер чистого дочернего компонента при неизменившихся props.
**Как используется / работает** — применяю после профилирования к дорогому компоненту со стабильными props и shallow comparison.
**Минусы и ограничения** — сравнение тоже стоит времени; новые объекты и callbacks обнуляют выгоду, а код становится сложнее.
...
**Как используется / работает** — использую для измеренно дорогой фильтрации либо для prop memoized-child, указывая полные dependencies.
**Минусы и ограничения** — это optimization hint, не гарантия; лишнее применение ухудшает читаемость и иногда медленнее вычисления.
...
**Для чего используется** — сохраняет identity функции, когда это влияет на memoized child или dependency effect.
**Как используется / работает** — передаю callback в memo-компонент или subscription и перечисляю значения, которые он читает.
**Минусы и ограничения** — сам по себе не ускоряет приложение; stale closure из неполных dependencies создаёт скрытые баги.
...
**Для чего используется** — хранит mutable значение или DOM-узел без запуска рендера.
**Как используется / работает** — применяю для focus, таймера, AbortController или предыдущего значения, обновляя ref в effect/handler.
**Минусы и ограничения** — изменение ref не обновляет экран; не стоит хранить там state, который должен быть виден пользователю.
...
**Как используется / работает** — запускаю effect после commit, возвращаю cleanup и перечисляю все реактивные зависимости.
**Минусы и ограничения** — не нужен для вычисления derived state; неверные dependencies вызывают loops, stale data и утечки.
...
**Для чего используется** — читает layout и синхронно исправляет DOM до показа кадра, избегая визуального скачка.
**Как используется / работает** — использую редко для измерения tooltip/popover или восстановления scroll-позиции.
**Минусы и ограничения** — блокирует paint и не подходит для обычных запросов; в SSR требует аккуратности.
...
**Как используется / работает** — оставляю input state срочным, а фильтрацию/навигацию запускаю внутри startTransition.
**Минусы и ограничения** — это не debounce и не отмена сети; нельзя применять для значения, контролирующего input.
...
**Как используется / работает** — передаю deferred query в тяжёлый список, пока текстовое поле сразу показывает свежий query.
**Минусы и ограничения** — отображаются слегка устаревшие данные; это не уменьшает число запросов без debounce и cancellation.
...
**Для чего используется** — передаёт общие данные без prop drilling, например theme, locale или текущего пользователя.
**Как используется / работает** — размещаю Provider близко к потребителям и разделяю contexts по частоте обновления.
**Минусы и ограничения** — изменение value ререндерит всех consumers; это не универсальная замена локальному или server state.
...
**Для чего используется** — controlled input даёт React полный контроль над значением, uncontrolled оставляет его DOM.
**Как используется / работает** — controlled выбираю для валидации и зависимого UI; uncontrolled с ref — для простой формы или интеграции.
**Минусы и ограничения** — controlled поля могут рендерить слишком часто; смешивать два источника истины опасно.
...
**Как используется / работает** — поднимаю state до ближайшего общего родителя и передаю value/handlers вниз.
**Минусы и ограничения** — подъём слишком высоко раздувает props и область ререндеров; иногда лучше composition или context.
...
**Для чего используется** — уменьшает связанность и лишние рендеры, удерживая state рядом с единственными потребителями.
**Как используется / работает** — начинаю с локального state и поднимаю/глобализирую его только при реальной общей потребности.
...
**Как используется / работает** — применяю TanStack Query/SWR для cache key, loading/error, invalidation и mutation.
**Минусы и ограничения** — нельзя путать с UI state; неверные keys или invalidation дают устаревший интерфейс.
...
**Для чего используется** — централизует сложный client state и делает transitions предсказуемыми через явные actions/reducers.
**Как используется / работает** — храню только действительно shared client state, описываю события actions-ами и обновляю state чистыми reducers; RTK уменьшает boilerplate.
**Минусы и ограничения** — для локального UI state и server cache это часто избыточно; слишком много global state скрывает ownership и усложняет изменение.
...
**Для чего используется** — даёт небольшой external store для shared client state с меньшим ceremony, чем Redux.
**Как используется / работает** — создаю focused stores и selectors, чтобы component подписывался только на нужный slice state.
**Минусы и ограничения** — простота не заменяет model design; без conventions store легко становится неявным глобальным mutable state.
...
**Для чего используется** — управляет server state: cache, loading/error, retries, refetching, invalidation и optimistic mutations.
**Как используется / работает** — query key включает все input parameters, mutation invalidates/updates affected key, а staleTime выбираю по freshness requirement.
**Минусы и ограничения** — это не global UI state manager; плохие keys, бесконтрольный retry или optimistic rollback дают stale/неверный UI.
...
**Для чего используется** — файловая модель Next.js для layouts, Server Components, streaming и серверного data fetching.
**Как используется / работает** — route segment содержит page, layout, loading и error; компоненты серверные по умолчанию.
**Минусы и ограничения** — нужны ясные границы client/server; миграция со старого Pages Router имеет ментальную стоимость.
...
**Как используется / работает** — page-файлы формируют routes, а данные загружаются через специальные data-fetching функции.
**Минусы и ограничения** — не получает новую RSC-модель; поддержка legacy-кода может усложнить единый подход.
...
**Для чего используется** — выполняет UI-компонент на сервере и не отправляет его JavaScript в браузер.
**Как используется / работает** — читаю БД или backend API прямо в async server component и передаю serializable props вниз.
**Минусы и ограничения** — нельзя использовать state, effects, browser APIs или event handlers; нельзя передавать произвольные объекты.
...
**Для чего используется** — добавляет интерактивность: state, события, эффекты и browser APIs.
**Как используется / работает** — добавляю директиву use client только на листе интерактивной части дерева.
**Минусы и ограничения** — всё импортируемое попадает в клиентский bundle; слишком высокий boundary раздувает его.
...
**Как используется / работает** — сервер получает данные, рендерит HTML, затем клиент hydrate-ит интерактивные части.
**Минусы и ограничения** — увеличивает TTFB и нагрузку сервера; cache strategy критична для масштабирования.
...
**Для чего используется** — рендерит и получает данные в браузере, что удобно для очень интерактивного private UI.
**Как используется / работает** — отдаем shell и JavaScript, затем fetch из клиента заполняет интерфейс.
**Минусы и ограничения** — медленнее полезный первый экран и хуже SEO; требуется продуманный loading/error UX.
